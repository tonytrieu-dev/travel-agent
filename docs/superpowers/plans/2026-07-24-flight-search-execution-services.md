...
- [ ] **Step 3: Delete the now-dead code from `trips_repository.py` (narrowed — see correction below)**

CORRECTION found during execution: `backend/app/agent/planner.py` still imports `flight_provider_name`, `get_recent_flight_results`, and `offer_summary` directly from `trips_repository.py` until Task 6 rewires it. Deleting all six functions in this task breaks `planner.py`'s module import (and every test that imports it) before Task 6 has run. Only delete `search_flights`, `_record_search_flights_run`, `_to_flight_result` here — these have no caller left once the route is rewired. Leave `get_recent_flight_results`, `offer_summary`, `flight_provider_name` in `trips_repository.py` for now (a deliberate, temporary duplication of `offer_summary`/`flight_provider_name` with `services/flight_search.py`); Task 6 removes `planner.py`'s dependency on the repository's copies and then deletes these three leftover functions from `trips_repository.py` as its own final step. Keep `_cheapest_first` and `_complete_flight_results` in `trips_repository.py` — `get_trip_snapshot` still calls `_cheapest_first` directly and is out of scope for this refactor. Remove only imports that become genuinely unused by deleting `search_flights`/`_record_search_flights_run`/`_to_flight_result` specifically (e.g. possibly `AgentRunStep`, `AgentStepKind`) — do not remove anything `get_recent_flight_results`/`offer_summary`/`flight_provider_name`/`get_trip_snapshot` still need (`FLIGHT_CACHE_TTL_MINUTES`, `record_event`, `ExecutionEventKind`, `TripRequest`, `FlightResultSource`, `FlightSearchResult`).
...
- [ ] **Step 3b: Finish Task 5's deferred cleanup — delete the leftover repository functions**

Now that `planner.py` no longer imports `get_recent_flight_results`/`offer_summary`/`flight_provider_name` from `trips_repository.py` (Step 3 above rewired it to `services/flight_search.py`), grep to confirm nothing else references them (`grep -rn "get_recent_flight_results\|repository\.offer_summary\|repository\.flight_provider_name" backend/app backend/tests`), then delete `get_recent_flight_results`, `offer_summary`, and `flight_provider_name` from `backend/app/repositories/trips_repository.py` — Task 5 deliberately left these in place because deleting them before this step broke `planner.py`. Remove any now-unused imports this final deletion leaves behind.
